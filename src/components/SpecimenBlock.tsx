...
export function SpecimenBlock({ type, children, isPrinting }: SpecimenBlockProps) {
  ...
  return (
    <section
      ...
      className="group relative border-b border-white/10 py-8 last:border-b-0"
    >
      {!isPrinting && hover && (
        <div className="absolute -top-6 left-0 flex items-center gap-2">
          <span className="font-ui rounded-lg bg-white/10 px-2 py-1 text-xs uppercase text-white/80">
            {BLOCK_LABELS[type]}
          </span>
          <button
            type="button"
            className="font-ui rounded-lg border border-white/20 bg-white/5 px-2 py-1 text-xs text-white/80 hover:bg-white/10"
            onClick={(e) => {
              e.stopPropagation();
